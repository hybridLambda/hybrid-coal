...
//vector < vector < vector < double > > > gijoe_matrix
	...
	for (size_t i_coal_hist=0;i_coal_hist<coal_hist.size();i_coal_hist++){
		...
		for (size_t i=0;i<all_w[i_coal_hist].size();i++){
			size_t current_enum;
			for (size_t node_i=0; node_i<my_sp_net->NodeContainer.size();node_i++){
				Node * current_node = my_sp_net->NodeContainer[node_i];
...
void Tree::build_coal_hist_mat( Net& my_sp_net ){
